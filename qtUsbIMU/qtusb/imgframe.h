...
struct DFrameStruct
{
    ...
    //byte* IMUDataBuffer;
    unique_ptr<byte> leftData ;
    unique_ptr<byte> rightData;
    unique_ptr<IMUDataStruct> IMUData;
    unique_ptr<byte> IMUDataBuffer;
...
private:
    //byte * imgBuf;
    int m_camNum;
    vector<unique_ptr<byte> > list_imgBuf;//error;//: ‘unique_ptr’ was not declared in this scope;
    int IMUDatawidth=2;
public:
    int m_width;
    int m_height;
